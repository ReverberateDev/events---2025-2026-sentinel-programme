...
# ...existing code...
def get_client_request(client_socket, buffer):
    """
    Return a tuple: (request_string or special flag, remaining_buffer)
    - If no full request yet, keep reading.
    - On timeout or client close, return (None, buffer).
    - On too-large payload, return ("TOO_BIG", b"").
    """
    def extract_from_buffer(buf: bytes):
        sep = buf.find(b'\r\n\r\n')
        if sep == -1:
            return None
        headers_part = buf[:sep].decode()
        content_length = 0
        for line in headers_part.split('\r\n'):
            if line.lower().startswith('content-length:'):
                content_length = int(line.split(':', 1)[1].strip())
                break
        total_len = sep + 4 + content_length
        if len(buf) >= total_len:
            req_bytes = buf[:total_len]
            remaining = buf[total_len:]
            return req_bytes.decode(), remaining
        return None

    # First, try to parse any complete request already in buffer
    parsed = extract_from_buffer(buffer)
    if parsed:
        return parsed

    window_start = time.monotonic()
    window_bytes = 0
    min_rate = (50 * 10 ** 3) # 50 KB/s
    while True:
        try:
        ...
        if elapsed >= 1.0 and (window_bytes / elapsed) < min_rate:
            print("Request rate too slow.")
            return "TOO_SLOW", b""
        if len(buffer) > config['max_request_size']:
            print("Request size exceeded maximum limit.")
            return "TOO_BIG", b""
        parsed = extract_from_buffer(buffer)
        if parsed:
            return parsed
...
def handle_request(client_socket):
    ...
    while True:
        request, buffer = get_client_request(client_socket, buffer)
        if request is None:
            break
        if request == "TOO_BIG":
            send_response(client_socket, '413 Payload Too Large', '<h1>413 Payload Too Large</h1>')
            break
        if request == "TOO_SLOW":
